<x-layout>
    ...
    <h3 class="text-gray-700 text-3xl font-medium">Clinic Record List</h3>
    ...
    <div class="mt-4">
        <div class="flex flex-wrap -mx-6">
            <x-card>
                ...
                <x-slot:count> {{ count($clinicRecords) }}</x-slot:count>
                Total Clinic Records
            </x-card>    
    ...
    <x-table-paginate.table-pagination>
        <x-slot:title>Record List</x-slot:title>
        ...
        <x-table-paginate.table>
            <thead>
                <tr>
                    ...
                    <x-table-paginate.th>Chief Complaints</x-table-paginate.th>
                    <x-table-paginate.th>Date</x-table-paginate.th>
                </tr>
            </thead>
            
            <tbody>
                @foreach ($employees as $employee)    
                    @if (count($employee->clinicRecords) > 0)
                        ...
                        <tr>
                            ...
                            <x-table-paginate.td>
                                ...
                                @foreach ($employee->clinicRecords as $records)
                                    <p>
                                        {{ $records->chief_complaints }}<br> (

                                        @foreach ($records->findings as $finding)
                                            {{ $finding->name }}
                                        @endforeach
                                        )
                                    </p>                              
